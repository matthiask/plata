...
class Country(models.Model):
    ...
    country_group = models.ForeignKey(
        CountryGroup,
        verbose_name=_('country group'),
        help_text=_('The country belongs to this group of countries for which your shipping provider charges the same.')
        )
...
class ShippingProvider(models.Model):
    ...
    remarks = models.TextField(
        ...
        help_text=_('internal information'),
        blank=True
        )
...
class Postage(models.Model):
    ...
    name = models.CharField(
        verbose_name=_('name'),
        max_length=31,
        help_text=_('How your shipping provider calls this class of packet, e.g. "Parcel XL".'),
        )
    ...
    price_external = models.DecimalField(
        ...
        max_digits=10, decimal_places=2,
        help_text=_('The price that you charge your customers, e.g. internal price plus packaging.'))
    ...
    max_3d = models.PositiveIntegerField(
        ...
        default=0,
        help_text=_('Maximum measure of length+width+height for this tariff. 0 = ignored')
        )
    ...
    def max_size_f(self):
        ...
        if d3:
            return '%s × %s × %s %s' % (self.max_length, self.max_width, self.max_height, LENGTH_UNIT)
        return '%d %s' % (size, LENGTH_UNIT)
    max_size_f.help_text = _('maximum size of length + width + height, formatted with unit')
