            ...
            "form_fields": [
                (
                    ...
                    forms.ModelMultipleChoiceField(
                        Product._default_manager.all(), required=True
                    ),
                )
            ],
...
class ModelTest(PlataTest):
    def test_00_test(self):
        ...
        self.assertRaises(
            ...
            lambda: self.assertRaisesWithCode(
                ValidationError, raise_validationerror, code="something"
            ),
        )
        ...
        self.assertRaises(
            NotImplementedError,
            lambda: self.assertRaisesWithCode(
                ValidationError, raise_notimplementederror, code="something"
            ),
        )
        ...
        self.assertRaises(
            Exception,
            lambda: self.assertRaisesWithCode(
                ValidationError, lambda: None, code="something"
            ),
        )
    ...
    def test_03_mixed_currencies(self):
        ...
        order.currency = "EUR"
        self.assertRaisesWithCode(
            ValidationError, lambda: order.modify_item(p2, 2), code="multiple_currency"
        )
    ...
    def test_05_order_status(self):
        ...
        self.assertRaisesWithCode(
            ValidationError,
            lambda: order.update_status(Order.CHECKOUT, "Checkout process has started"),
            code="order_empty",
        )
        ...
        order.update_status(Order.CONFIRMED, "Order has been confirmed")

        # Should not be possible to modify order once checkout process
        # has started
        self.assertRaisesWithCode(
            ValidationError, lambda: order.modify_item(product, 2), code="order_sealed"
        )
    ...
    def test_06_order_percentage_discount(self):
        ...
        self.assertAlmostEqual(
            order.total, item.discounted_subtotal + item2.discounted_subtotal
        )
    ...
    def test_07_order_amount_discount(self):
        ...
        self.assertAlmostEqual(
            discounted1.line_item_discount, discount.value / tax_factor / 8 * 3
        )
        self.assertAlmostEqual(
            order.total,
            discounted1.discounted_subtotal
            + discounted2.discounted_subtotal
            + order.items_tax,
        )
    ...
    def test_09_selective_discount(self):
        ...
        self.assertEqual(order.items.count(), 2)
        self.assertEqual(
            1, len([item for item in order.items.all() if item._line_item_discount])
        )
    ...
    def test_16_payment(self):
        ...
        payment = order.payments.create(currency=order.currency, amount=100)
        ...
        order.payments.create(
            currency=order.currency, amount=Decimal("139.70"), authorized=timezone.now()
        )
    ...
    def test_17_stocktransactions(self):
        ...
        period = Period.objects.create(name="Period 1", start=timezone.now())
        # Create a period which has been superceeded by Period 1
        Period.objects.create(
            name="Period 0",
            start=timezone.make_aware(
                datetime(2000, 1, 1, 0, 0), timezone.get_default_timezone()
            ),
        )
        ...
        Period.objects.create(
            name="Period 2",
            start=timezone.make_aware(
                datetime(2030, 1, 1, 0, 0), timezone.get_default_timezone()
            ),
        )

        s = StockTransaction.objects.create(
            product=product, type=StockTransaction.INITIAL, change=10
        )
        ...
        StockTransaction.objects.create(
            product=product, type=StockTransaction.CORRECTION, change=-3
        )

        self.assertEqual(StockTransaction.objects.items_in_stock(product), 7)

        StockTransaction.objects.create(
            product=product, type=StockTransaction.SALE, change=-2
        )

        StockTransaction.objects.create(
            product=product, type=StockTransaction.PURCHASE, change=4
        )
    ...
    def test_24_uninitialized_order(self):
        ...
        plata.reporting.order.invoice_pdf(
            PlataPDFDocument(BytesIO()), Order.objects.create()
        )
    ...
    def test_26_discounts(self):
        ...
        product = Product.objects.create(name="Ein Paar Hosen")
    ...
    def test_29_product_with_several_orderitems(self):
        ...
        self.assertRaises(
            ValidationError, order.modify_item, product, relative=1, force_new=False
        )
        self.assertEqual(order.items.count(), 3)
